...
def health():
    return {
        ...
        "queue": "idle",
        "last_job": recent_jobs(1)[0]["job_type"] if recent_jobs(1) else None,
        "files_exported": sum(len(j.get("exports", [])) for j in recent_jobs(10)),
        "failures": sum(int(j.get("failures", 0)) for j in recent_jobs(10)),
    }


@app.post("/api/ixl/connect")
def connect_ixl():
    # Minimal placeholder: for the hosted version, this endpoint should
    # start your real connect/auth flow. For now it clearly signals the next step.
    raise HTTPException(
        status_code=501,
        detail="Hosted IXL connect flow not implemented yet. Build this next as a guided login/session-capture flow."
    )
